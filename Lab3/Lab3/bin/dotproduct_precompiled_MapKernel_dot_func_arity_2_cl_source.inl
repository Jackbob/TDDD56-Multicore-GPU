...
class CLWrapperClass_dotproduct_precompiled_MapKernel_dot_func_arity_2
{
public:
...
__kernel void dotproduct_precompiled_MapKernel_dot_func_arity_2(__global float *element1, __global float *element2,  __global float* output, size_t w, size_t n, size_t base)
{
	...
	while (i < n)
	{
		...
		output[i] = dot_func(element1[i], element2[i]);
		i += gridSize;
	}
}
)###");
		...
		for (skepu2::backend::Device_CL *device : skepu2::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel = clCreateKernel(program, "dotproduct_precompiled_MapKernel_dot_func_arity_2", &err);
			CL_CHECK_ERROR(err, "Error creating map kernel '" << "dotproduct_precompiled_MapKernel_dot_func_arity_2" << "'");
	...
	(
		...
		skepu2::backend::DeviceMemPointer_CL<float> *element1, skepu2::backend::DeviceMemPointer_CL<float> *element2,  skepu2::backend::DeviceMemPointer_CL<float> *output,
		size_t w, size_t n, size_t base
	)
	{
		skepu2::backend::cl_helpers::setKernelArgs(kernels(deviceID), element1->getDeviceDataPointer(), element2->getDeviceDataPointer(),  output->getDeviceDataPointer(), w, n, base);
