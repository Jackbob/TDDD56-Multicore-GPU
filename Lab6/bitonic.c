...
void runKernel(cl_kernel kernel, int threads, cl_mem data, unsigned int outer_length, unsigned int inner_length)
{
  ...
  // printf("Global size %ld local work size %ld \n", globalWorkSize, localWorkSize);
	
	// set the args values
	ciErrNum  = clSetKernelArg(kernel, 0, sizeof(cl_mem),  (void *) &data);
	ciErrNum |= clSetKernelArg(kernel, 1, sizeof(cl_uint), (void *) &outer_length);
  ciErrNum |= clSetKernelArg(kernel, 2, sizeof(cl_uint), (void *) &inner_length);
