...
void runKernel(cl_kernel kernel, int threads, cl_mem data, unsigned int length, cl_mem local_data)
{
  ...
  ciErrNum |= clSetKernelArg(kernel, 2, localWorkSize * sizeof(unsigned int), NULL);
...
int find_max_gpu(unsigned int *data, unsigned int length)
{
  ...
  local_data = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, length * sizeof(unsigned int), NULL, &ciErrNum);

  printCLError(ciErrNum,7);

	// ********** RUN THE KERNEL ************
	runKernel(gpgpuReduction, length, io_data, length, local_data);
  ...
  // How to get number of groups that just executed kernel?
  // Now hardcoded for Signal & bild-labb
  if (2560 < length)
    find_max_cpu(data, 2560);
  else find_max_cpu(data, 100);
...
int main( int argc, char** argv) 
{
  ...
  if (data_cpu[0] != data_gpu[0])
    {
      printf("Wrong value at position 0, should be %ld \n", data_cpu[0]);
      closeOpenCL();
      return(1);
    }
